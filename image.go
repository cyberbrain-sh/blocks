...
func AddImageProperties(b *Block, size *int, transcription *string, publicURL *string, enriched bool) error {
	if b == nil {
...
// RenderImageProperties renders image properties in a human-readable format
func RenderImageProperties(b Block) string {
	...
	if hasPublicURL && publicURL != "" {
		parts = append(parts, fmt.Sprintf("![Image](%s)", publicURL))
	}
...
// GetImageProperties returns a list of all image property keys
func GetImageProperties() []string {
	return []string{
		PropertyKeySize,
